...
class TrocaDeMensagens:
    def __init__(self):
        self.mensagens = ["Mensagem inicial"] # Inicializa a fila de mensagens com uma mensagem
        self.mutex = threading.Lock()

    def enviaMensagem(self, mensagem, thread_id):
        with self.mutex:
            self.mensagens.append(mensagem) # Adiciona mensagem
            print(f'Thread {thread_id} enviou uma mensagem')

    def recebeMensagem(self, thread_id):
        with self.mutex:
            if self.mensagens:
                print(f'Thread {thread_id} recebeu uma mensagem')
                return self.mensagens.pop(0) # Retira a primeira mensagem (FIFO)
            return None
...
# Inicializa a fila das mensagens
filaDeMensagens = TrocaDeMensagens()
lock = threading.Lock()
...
def procuraCartas(thread_id, vetor_posicoes):
    ...
    for pos in posicoes:
        ...
        try:
            while True:
                # Recebe ou não uma mensagem
                mensagem = filaDeMensagens.recebeMensagem(thread_id)
                ...
                if mensagem:
                    ...
                    # Envia uma mensagem ao terminar de pegar uma carta
                    filaDeMensagens.enviaMensagem((thread_id, pos), thread_id)
                    break
                else:
                    continue      
        finally:
